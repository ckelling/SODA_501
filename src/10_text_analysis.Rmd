...
locationNames <- matrix(c("ann", "arbor", "michigan"),ncol=1,byrow=FALSE)
colnames(locationNames) <- "word"
locationNames <- tbl_df(locationNames)

AnnArbor %>%
  filter(!word %in% locationNames$word,
         str_detect(word, "[a-z]"))

wordcloud(words=AnnArbor$word, freq=AnnArbor$freq, min.freq = 0.0003,
